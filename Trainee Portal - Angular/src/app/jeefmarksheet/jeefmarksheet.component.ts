...
@Component({
  selector: 'app-jeefmarksheet',
  templateUrl: './jeefmarksheet.component.html',
  styleUrls: ['./jeefmarksheet.component.css']
})
export class JeefmarksheetComponent implements OnInit {
  ...
  insertdetails(insert:any){
    this.is.addJEEMarksheet(insert.value).subscribe();
  ...
  updateLoginpage(data:any){
    this.ps1.updateJEEMarksheet(data.value).subscribe();
    alert('You Have successfully updated student marks');
    window.location.reload();
  }

  deleteLoginpage(data:any){
    this.ps.deleteJEEMarksheet(data.value).subscribe();
